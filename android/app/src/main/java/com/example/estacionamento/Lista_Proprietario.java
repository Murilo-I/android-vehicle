...
public class Lista_Proprietario extends AppCompatActivity {
    ...
    public void listarTodosProprietarios(String resposta) {
        final ArrayList<Proprietario> lista = new ArrayList<Proprietario>();
        try {
            JSONArray jsonArray = new JSONArray(resposta);
            for (int i = 0; i < jsonArray.length(); i++) {
                Proprietario p = new Proprietario();

                p.setId(jsonArray.getJSONObject(i).getInt(String.valueOf("id_proprietario")));
            ...
            AdapterProprietario adapter = new AdapterProprietario(context,
                    R.layout.adapter,
                    R.id.id_proprietario,
                    lista);
